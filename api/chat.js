...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const nimRequest = {
      ...
      // Include usage in stream so we get proper chunk accounting
        ...
        lines.forEach(line => {
          ...
          try {
            ...
            if (data.choices?.[0]?.delta) {
              ...
              if (isKimi) {
                // Kimi sends thinking already embedded in delta.content with <think> tags.
                // delta.reasoning mirrors it redundantly — delete it and pass content through.
                delete delta.reasoning;
                delete delta.reasoning_content;
                // If delta has no content at all, set empty string to avoid null issues
                if (delta.content === undefined) delta.content = '';
              } else {
                // DeepSeek-style: reasoning comes in reasoning_content, response in content.
