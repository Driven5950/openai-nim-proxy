  ...
  try {
    ...
    if (stream) {
        ...
        lines.forEach(line => {
          ...
          try {
            ...
            if (data.choices?.[0]?.delta) {
              ...
              // Determine if content is meaningfully present (not null/undefined/empty)
              const hasContent = content !== null && content !== undefined && content !== '';
              const hasReasoning = reasoning !== null && reasoning !== undefined && reasoning !== '';

              let output = '';

              if (hasReasoning && hasContent) {
                // Both fields present in the same chunk: model is self-tagging inside content.
                // content already contains the <think> markup — use it directly, drop reasoning_content.
                output = content;
              } else if (hasReasoning) {
                // Only reasoning_content — model uses separate field (DeepSeek-style).
                // Wrap it ourselves.
                if (!inReasoning) { output += '<think>'; inReasoning = true; }
                output += reasoning;
              } else if (hasContent) {
                // Only content — could be the tail of self-tagged thinking or normal response.
      ...
      const choices = response.data.choices.map(choice => {
        ...
        // If content already contains <think> tags, model is self-tagging — don't also prepend reasoning_content
