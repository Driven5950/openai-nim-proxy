...
app.get('/v1/models', (req, res) => {
  res.json({ object: 'list', data: Object.keys(MODEL_MAPPING).map(id => ({ id, object: 'model', created: Date.now(), owned_by: 'nvidia-nim-proxy' })) });
});
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    if (!nimModel) {
      ...
      if (lower.includes('gpt-4') || lower.includes('claude-opus') || lower.includes('405b')) nimModel = 'meta/llama-3.1-405b-instruct';
      else if (lower.includes('claude') || lower.includes('gemini') || lower.includes('70b')) nimModel = 'meta/llama-3.1-70b-instruct';
    ...
    const headers = { 'Authorization': `Bearer ${NIM_API_KEY}`, 'Content-Type': 'application/json' };
    ...
    if (stream) {
      ...
      const response = await axios.post(`${NIM_API_BASE}/chat/completions`, nimRequest, { headers, responseType: 'stream' });
      ...
      const choices = response.data.choices.map(choice => {
        ...
        if (reasoning && !content.includes('<think>')) content = '<think>\n' + reasoning + '\n</think>\n\n' + content;
        return { index: choice.index, message: { role: choice.message.role, content }, finish_reason: choice.finish_reason };
      });
      res.json({ id: `chatcmpl-${Date.now()}`, object: 'chat.completion', created: Math.floor(Date.now() / 1000), model, choices, usage: response.data.usage || {} });
    }
  } catch (error) {
    console.error('Proxy error:', error.message);
    res.status(error.response?.status || 500).json({ error: { message: error.message || 'Internal server error', type: 'invalid_request_error' } });
  }
});

app.all('*', (req, res) => res.status(404).json({ error: { message: `Endpoint ${req.path} not found`, type: 'invalid_request_error' } }));
